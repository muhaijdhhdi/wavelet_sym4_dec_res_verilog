...
// 
// 输入：16路并行采样数据，打包为256位 (16×16bit Q16.0)
// 输出：去除基线后的信号，打包为256位 (16×16bit Q16.0)
// 
// 数据格式：
//   din[15:0]   = 通道0 (最早采样点)
//   din[31:16]  = 通道1
//   ...
//   din[255:240] = 通道15 (最晚采样点)
// 
// 流水线延迟分析：
...
module wavelet_baseline_removal_top #(
    parameter DATA_WIDTH     = 16,      // 单通道数据位宽 (Q16.0)
    parameter COEF_WIDTH     = 25,      // 系数位宽 (Q2.23)
    parameter COEF_FRAC      = 23,      // 系数小数位
    parameter INTERNAL_WIDTH = 48,      // 内部计算位宽 (Q25.23)
    parameter TOTAL_DELAY    = 154,     // 总延迟周期数（算法延迟+物理延迟）
    
    //==========================================================================
    // sym4 分解低通滤波器系数 (Q2.23)
    //==========================================================================
    ...
    input  wire [DATA_WIDTH*16-1:0]          din,          // 256位
    
    // 输出：基线，打包为256位
    output wire                              baseline_valid,
    output wire [DATA_WIDTH*16-1:0]          baseline,     // 256位
    
    // 输出：去除基线后的信号，打包为256位
    output wire [DATA_WIDTH*16-1:0]          signal_no_baseline  // 256位
);
