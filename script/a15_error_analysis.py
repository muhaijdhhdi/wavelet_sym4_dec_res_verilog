...
def golden_model_floating_L5(data, h):
    block_size = 1
    num_cycles = len(data) // block_size
    res = []
    x_hist = data[0:7]
...
# L4
# 最终的理想数据
l4_out = golden_model_floating_L4(l3_out, h_dec)
print(f"L4 输出长度: {len(l4_out)}")

# L5
l5_out = golden_model_floating_L5(l4_out, h_dec)
# ==========================================
# 5. 读取 Verilog L5 输出文件
# ==========================================
def str2dec(hex_str):
    val = int(hex_str, 16)
...
# 这里的路径改为 a5_output.txt
OUTPUT_FROM_VERILOG = "D:/project/zu9_modifield+pulse_iir/PL/FMC_FH8052_zu9/FMC_FH8052.srcs/sources_1/imports/src/simulation/a5_output.txt"
sim_results = []
...
# 6. 结果比对与分析
# ==========================================
ideal_data_final = l5_out
# 对齐数据长度
min_len = min(len(ideal_data_final), len(sim_results))
print(f"对比长度: {min_len}")

print(f"硬件 L5 前10点: {sim_results[:10]}")
print(f"Python L5 前10点: {ideal_data_final[:10]}")



# # 使用最佳偏移进行最终对比
a5_v = sim_results[ : min_len]
a5_p = ideal_data_final[:min_len]
...
print("=" * 60)
print(f"L5 五级级联仿真误差分析")
print("-" * 60)
print(f"硬件输出示例: {a5_v[10:15]}")
print(f"理想模型示例: {a5_p[10:15]}")
print("-" * 60)
